...
def _resolve_deepseek_api_key(inline_key: str = "", session_id: str = "") -> str:
    inline_key = str(inline_key or "").strip()
    if inline_key:
        return inline_key

    session_id = str(session_id or "").strip()
    stored_key = str(_SESSION_KEYS.get(session_id, {}).get("api_key") or "").strip()
    if stored_key:
        return stored_key
...
def create_app(settings: Optional[Settings] = None) -> Any:
    ...
    async def generate_insight(run_id: str, payload: Dict[str, Any] = Body(default={})) -> Any:
        ...
        api_key = _resolve_deepseek_api_key(inline_key=inline_key, session_id=session_id)
        if not api_key:
