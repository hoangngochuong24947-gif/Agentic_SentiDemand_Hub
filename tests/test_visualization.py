...
class TestVisualizationGallery:
    ...
    def test_deepseek_env_key_fallback_for_legacy_insight_generation(self, tmp_path: Path, monkeypatch):
        settings = Settings(
            paths=PathConfig(
                output_base=tmp_path / "outputs",
                visualization_base=tmp_path / "vis_outputs",
        ...
        def fake_call(api_key, prompt):
            ...
            return {"response": {"id": "mock"}, "content": "## 建议\n- 环境变量"}

        monkeypatch.setenv("DEEPSEEK_API_KEY", "sk-env-secret")
        monkeypatch.setattr(gallery, "_call_deepseek", fake_call)
        ...
        insight_resp = client.post(f"/api/runs/{run_id}/insights/generate", json={})
        assert insight_resp.status_code == 200
        assert captured["api_key"] == "sk-env-secret"
