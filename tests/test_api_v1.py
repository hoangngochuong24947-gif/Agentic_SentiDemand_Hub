...
def test_generate_insights_uses_env_key_when_session_missing(monkeypatch, tmp_path):
    client, _ = _client_with_run(tmp_path)
    captured = {}
    ...
    assert response.json()["structured_advice"]["context"]["provider"] == "deepseek"


def test_generate_insights_prefers_session_key_over_env(monkeypatch, tmp_path):
    client, _ = _client_with_run(tmp_path)
    captured = {}
    session_id = "session-001"
    gallery._SESSION_KEYS[session_id] = {"api_key": "sk-session-secret", "updated_at": "now"}
